#!/usr/bin/env bash
# run_experiment_voyage_law2_hybrid_10.sh
#
# Experiment: voyage-law-2 (1024-d, legal-domain) + BM42 sparse,
# hybrid search via Qdrant RRF fusion, first 10 contracts.
#
# VoyageAI voyage-law-2 is fine-tuned for legal document retrieval —
# strong candidate for CUAD. Uses asymmetric embeddings: document chunks
# are embedded with input_type="document", queries with input_type="query".
#
# Prerequisites:
#   VOYAGE_API_KEY must be set in cuad-demo-quadrant/.env
#   Get a free API key at: https://www.voyageai.com/
#
# Compare against:
#   run_experiment_bgelarge_hybrid_50.sh  (BAAI/bge-large-en-v1.5, 1024-d, HF)
#   run_experiment_minilm_hybrid_50.sh   (all-MiniLM-L6-v2, 384-d, local)
#
# Usage:
#   bash tests/eval/run_experiment_voyage_law2_hybrid_10.sh
...
# VoyageAI free tier: 3 requests/min → sleep 20 s between API calls
ENCODE_SLEEP_S=20
SERVER_PORT=8006
